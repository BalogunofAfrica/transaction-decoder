...
impl Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            ...
            Error::ParseFailed(ref msg) => write!(f, "parse failed: {}", msg)
        }
    }
}
...
impl Transaction {
     pub fn compute_txid(&self) -> Txid {
            ...
            self.version.consensus_encode(&mut tx_id_data).expect("writing to vec should not fail");
            self.inputs.consensus_encode(&mut tx_id_data).expect("writing to vec should not fail");
            self.outputs.consensus_encode(&mut tx_id_data).expect("writing to vec should not fail");
            self.lock_time.consensus_encode(&mut tx_id_data).expect("writing to vec should not fail");
            Txid::from_raw_transaction(tx_id_data)
     }
}
...
impl Txid {
    ...
    pub fn from_raw_transaction(bytes: Vec<u8>) -> Txid {
...
#[derive(Debug, Serialize)]
struct CompactSize(pub u64);

#[derive(Debug)]
pub struct Amount(u64);

impl Amount {
    pub fn from_sats(sats: u64) -> Amount {
        Amount(sats)
    }
}
            ...
            match segwit_flag {
                1 => {
                    ...
                    if !inputs.is_empty() && inputs.iter().all(|input| input.witness.is_empty()) {
                        Err(Error::ParseFailed("witness flag set but no witness present"))
                    } else {
                        Ok(Transaction{
                            version,
                            inputs,
